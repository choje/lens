...
NlmToLensConverter.Prototype = function () {
    ...
    this.bodyNodes = function (state, children, options) {
        console.log('coverter bodyNodes children', children);
        var nodes = [], node;
        for (var i = 0; i < children.length; i++) {
            var child = children[i];
            var type = util.dom.getNodeType(child);
            console.log('coverter bodyNodes type', type);
            console.log('coverter bodyNodes child', child);
        ...
        console.log('coverter bodyNodes nodes', nodes);
        return nodes;
    };
    ...
    this._bodyNodes["table"] = function (state, child) {
        return this.table(state, child);
    };

    this.table = function (state, child) {


        console.log('coverter table child',child);
        var doc = state.doc;
        console.log('converter table chlidren ' , util.dom.getChildren(child));
        var childNodes = this.trGroup(state, util.dom.getChildren(child));
        var htmTableId = state.nextId("html_table");
        var tableNode = {
            "type": "html_table",
            "id": htmTableId,
        ...
        console.log('coverter bodyNodes table childNodes',childNodes);
    ...
    this.trGroup = function (state, child) {
        console.log('coverter trGroup child', child);
        var nodes = [];

        for (var i = 0; i <child.length; i++) {

            console.log('coverter trGroup tr', i, child[i]);
            nodes[i] = this.tr(state, child[i]);
        }
        console.log('coverter trGroup nodes', JSON.stringify(nodes));
        return nodes;
    ...
    this.tr = function (state, child) {
        var doc = state.doc;
        var childNodes = this.tdGroup(state, util.dom.getChildren(child));
        var html_tabl_tr_id = state.nextId("html_table_tr");
        var trNode = {
            "type": "html_table_tr",
            "id": html_tabl_tr_id,
            "source_id": child.getAttribute("id"),
        ...
        console.log('coverter bodyNodes tr childNodes',childNodes);
        //doc.create(trNode);
        return trNode;
        }

    this.tdGroup = function (state, child) {
        console.log('coverter tdGroup child', child);
        var nodes = [];

        for (var i = 0; i <child.length; i++) {

            console.log('>>coverter tdGroup td', i, child[i]);
            nodes[i] = this.td(state, child[i]);
        }
    ...
    this.td = function (state, child) {
        console.log('converter td', child);
        if (child !== undefined) {
            var doc = state.doc;
            var childNodes = util.dom.getChildren(child);


            console.log('>>coverter bodyNodes td childNodes', childNodes);
            for (var i; i < childNodes.length; i++) {
                var childNode = this.paragraphGroup(state, childNodes[i]);
                var html_tabl_td_id = state.nextId("html_table_td");
                var tdNode = {
                    "type": "html_table_td",
                    "id": html_tabl_td_id,
                    "source_id": child.getAttribute("id"),
                    "label": "",
                    "children": _.pluck(childNode, 'id')
                };
                doc.create(tdNode);
                return tdNode;
            }
        ...
        console.log('coverter bodyNodes node',node);
        if (node[0] !== undefined) {
    ...
    this.tableWrap = function (state, tableWrap) {
        console.log('converter tableWrape  tableWrap', tableWrap);
        var doc = state.doc;
        ...
        /* var content = {};
         ...
         for (var i = 0; i < trs.ength; i++) {
             console.log('conveter : tableWrap : tr : ', trs[i]);
             content[i] = this.bodyNodes(state, util.dom.getChildren(trs[i]));
         }*/
        var nodes = [];
        var children = util.dom.getChildren(childNodes);

        console.log('converter tableWrap childNodes', childNodes);

        tableNode.children = childNodes;
